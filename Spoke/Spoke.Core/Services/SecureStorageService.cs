...
/// <summary>
/// Service for secure storage of sensitive data using encryption
/// </summary>
public class SecureStorageService
{
    ...
    public SecureStorageService(string encryptionKey, ISecureStorage? storage = null)
    {
        _encryptionKey = encryptionKey ?? throw new ArgumentNullException(nameof(encryptionKey));
        _storage = storage ?? new PlatformSecureStorage();
    }
    ...
    public async Task<bool> StoreAsync<T>(string key, T data)
    {
        try
        {
            ...
            await _storage.SetAsync(key, Convert.ToBase64String(encryptedData));
            return true;
        }
        catch
        {
    ...
    public async Task<T?> RetrieveAsync<T>(string key)
    {
        try
        {
            var encryptedBase64 = await _storage.GetAsync(key);
    ...
    /// </summary>
    private class PlatformSecureStorage : ISecureStorage
    {
        public Task SetAsync(string key, string? value)
        {
#if ANDROID || IOS || MACCATALYST || WINDOWS
            // Use platform SecureStorage when available
            return SecureStorage.Default.SetAsync(key, value);
#else
            // Fallback for testing - use in-memory storage
            TestSecureStorage.Instance.SetAsync(key, value);
...
#else
            // Fallback for testing - use in-memory storage
            return TestSecureStorage.Instance.GetAsync(key);
#endif
        }
    }

    /// <summary>
    /// Test implementation of secure storage for unit testing
    /// </summary>
    public class TestSecureStorage : ISecureStorage
    {
        private readonly Dictionary<string, string> _storage = new();
        public static TestSecureStorage Instance { get; } = new();
